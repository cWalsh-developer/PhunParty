    ...
    try:
        for question_id in ["Q001", "Q002"]:
            ...
            if options:
                ...
                print(f"Hex representation: {binascii.hexlify(options.encode('utf-8') if isinstance(options, str) else str(options).encode('utf-8'))}")
...
def suggest_fixes():
    ...
    print("\n=== Potential Fixes ===")
    print("1. If BOM detected: UPDATE questions SET question_options = LTRIM(question_options, '\uFEFF') WHERE question_id IN ('Q001', 'Q002');")
    print("2. If whitespace issues: UPDATE questions SET question_options = TRIM(question_options) WHERE question_id IN ('Q001', 'Q002');")
    print("3. If encoding issues: Check database encoding and client encoding")
    print("4. If the JSON is actually valid, the issue might be in the Python JSON library or data retrieval")
